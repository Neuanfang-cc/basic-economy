...
public class BalanceAdminSetCommand {
    public static void register(CommandDispatcher<ServerCommandSource> serverCommandSourceCommandDispatcher,
        ...
        serverCommandSourceCommandDispatcher.register(CommandManager.literal("balanceadmin")
                .requires(source -> Permissions.check(source.getPlayer(), (BasicEconomy.MOD_ID + "balanceadmin.set"), 2))
    ...
    private static int run(ServerCommandSource source, ServerPlayerEntity target, float amount) {
        ...
        EconomyAPI.setBalance(target_uuid, amount);
        double balance_after = EconomyAPI.getBalance(target_uuid);

        if (balance_before != balance_after) {
            source.sendFeedback(() -> Text.translatable("balanceadmin.set.success", target_name, balance), true);
            return 1;
        } else {
